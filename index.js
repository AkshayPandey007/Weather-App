...
function append(data)
{
    ...
    min.innerText=`Min-temp: ${Math.floor(data.main.temp_min-270)} C`
    min.style="color:blue;font-size:25px"

    let max = document.createElement("p")
    max.innerText=`Max-temp: ${Math.ceil(data.main.temp_max-270)} C`
    max.style="color:blue;font-size:25px"

    let current = document.createElement("p")
    current.innerHTML=`current : ${Math.ceil(data.main.temp-270)} C`
